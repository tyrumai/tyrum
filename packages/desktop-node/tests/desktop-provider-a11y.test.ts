...
describe("DesktopProvider (a11y)", () => {
  it.skipIf(process.platform !== "linux")(
    "snapshot(include_tree) uses injected a11y backend when available",
    async () => {
  ...
  it.skipIf(process.platform !== "linux")(
    "snapshot(include_tree) prunes the tree to max_nodes",
    async () => {
      const backend = new MockDesktopBackend();
      const permissions = {
        desktopScreenshot: true,
  ...
  it.skipIf(process.platform !== "linux")(
    "query(a11y) uses the a11y backend without screen capture",
    async () => {
      const backend = new MockDesktopBackend();
      const permissions = {
        desktopScreenshot: true,
  ...
  it.skipIf(process.platform !== "linux")("act(a11y) uses the a11y backend", async () => {
    const backend = new MockDesktopBackend();
    const permissions = {
      desktopScreenshot: true,
      desktopInput: true,
