...
export interface PlatformConnectionPageProps {
  core: OperatorCore;
}

export function PlatformConnectionPage({ core }: PlatformConnectionPageProps) {
    ...
    return (
      <div className="grid gap-6">
        ...
        <Alert
          variant="warning"
          title="Not available"
          description="Platform connection controls are only available in the desktop app."
        />
      </div>
    );
  }
  ...
  useEffect(() => {
    ...
    void api.getConfig().then((cfg) => {
      ...
      if (typeof embedded?.["port"] === "number") setPort(embedded["port"] as number);

      const remote = c?.["remote"] as Record<string, unknown> | undefined;
      if (typeof remote?.["wsUrl"] === "string") setRemoteUrl(remote["wsUrl"] as string);
    ...
    const unsubscribe = api.onStatusChange((s) => {
      ...
      if (typeof info["gatewayStatus"] === "string") {
  ...
  const startGateway = async () => {
    ...
    try {
      ...
      setGatewayError(formatErrorMessage(error));
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="grid gap-6">
      ...
      <Tabs value={tab} onValueChange={(value) => setTab(value as Tab)}>
        ...
        <TabsContent value="operator">
          <ConnectPage core={core} mode="desktop" hideHeader />
        </TabsContent>
      </Tabs>
    </div>
  );
