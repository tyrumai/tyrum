...
import {
  ConversationQueueSignalDal,
  ConversationQueueInterruptError,
} from "../../src/modules/conversation-queue/queue-signal-dal.js";
...
describe("AgentRuntime conversation queue modes", () => {
  ...
  it("steer cancels pending tool calls and injects the steer message", async () => {
    homeDir = await mkdtemp(join(tmpdir(), "tyrum-conversation-scope-steer-"));
    ...
    await new AgentConfigDal(container.db).set({
      ...
      createdBy: { kind: "test" },
      reason: "conversation queue steer test",
    });
  ...
  it("interrupt aborts at the next tool boundary", async () => {
    homeDir = await mkdtemp(join(tmpdir(), "tyrum-conversation-scope-interrupt-"));
    container = await createContainer({ dbPath: ":memory:", migrationsDir, tyrumHome: homeDir });
    const agentId = await container.identityScopeDal.ensureAgentId(DEFAULT_TENANT_ID, "default");
    await new AgentConfigDal(container.db).set({
      tenantId: DEFAULT_TENANT_ID,
      ...
      createdBy: { kind: "test" },
      reason: "conversation queue interrupt test",
    });
    ...
    ).rejects.toBeInstanceOf(ConversationQueueInterruptError);
