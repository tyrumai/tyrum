...
describe("AgentRuntime approval/permission scenarios (e2e)", () => {
  ...
  it("does not cancel approved approvals when resume_token is missing (resumes using context token)", async () => {
    homeDir = await mkdtemp(join(tmpdir(), "tyrum-perms-agent-"));
    container = await createContainer({ dbPath: ":memory:", migrationsDir, tyrumHome: homeDir });
    ...
    const turnPromise = runtime.turn({
      channel: "test",
      thread_id: "thread-approval-approved-missing-token-1",
      message: "run a command",
    });
    ...
    expect(pending.prompt).toContain("tool.exec");
    expect(pending.status).toBe("pending");
    expect(pending.run_id).toBeTruthy();

    const ctx = pending.context as Record<string, unknown>;
    expect(typeof ctx["resume_token"]).toBe("string");
    expect((ctx["resume_token"] as string).trim().length).toBeGreaterThan(0);

    await container.db.run("UPDATE approvals SET resume_token = '' WHERE id = ?", [pending.id]);

    const approved = await container.approvalDal.respond(pending.id, true);
