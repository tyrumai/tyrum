...
export async function seedExecutionScope(db: SqlRunner, ids: ExecutionScopeIds): Promise<void> {
  await db.run(
    `INSERT INTO turn_jobs (job_id, conversation_key, lane, status, trigger_json, input_json, latest_turn_id)
     VALUES (?, ?, ?, 'running', ?, ?, ?)`,
    [ids.jobId, "agent:agent-1:thread:thread-1", "main", "{}", "{}", ids.runId],
  );

  await db.run(
    `INSERT INTO turns (turn_id, job_id, conversation_key, lane, status, attempt)
     VALUES (?, ?, ?, ?, 'running', 1)`,
