...
describe("AgentRuntime - conversation lifecycle and policy", () => {
  ...
  it("scopes conversation cleanup to the current agentId", async () => {
    ...
    const deleteSpy = vi.spyOn(container.conversationDal, "deleteExpired").mockResolvedValue(0);
  ...
  it("passes work conversation routing fields to tool execution audit context", async () => {
    homeDir = await mkdtemp(join(tmpdir(), "tyrum-agent-runtime-"));
    container = await createContainer({
      dbPath: ":memory:",
      migrationsDir,
