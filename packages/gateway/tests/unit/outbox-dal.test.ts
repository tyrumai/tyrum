...
describe("OutboxDal", () => {
  ...
  it("delivers broadcast rows to all consumers and directed rows only to target", async () => {
    const outbox = setup();
    await outbox.enqueue(DEFAULT_TENANT_ID, "ws.broadcast", { hello: "world" });
    ...
    const a = await outbox.poll(DEFAULT_TENANT_ID, "edge-a", 100);
    const b = await outbox.poll(DEFAULT_TENANT_ID, "edge-b", 100);
    ...
    await outbox.ackConsumerCursor(DEFAULT_TENANT_ID, "edge-a", a[a.length - 1]!.id);
    expect(await outbox.poll(DEFAULT_TENANT_ID, "edge-a", 100)).toHaveLength(0);
  });
});
