...
describe("migration runners", () => {
  ...
  it("treats renamed sqlite migrations as already applied", () => {
    return withTempMigrations(
      {
        "020_node_pairing_trust_allowlist.sql": "THIS IS NOT SQL",
      },
      (dir) => {
        const sqlite = createDatabase(":memory:");
        try {
          ...
          sqlite
            ...
            .run("019_node_pairing_trust_allowlist.sql");

          expect(() => migrate(sqlite, dir)).not.toThrow();

          const migRes = sqlite
            .prepare("SELECT name FROM _migrations WHERE name = ?")
            .all("020_node_pairing_trust_allowlist.sql") as Array<{ name: string }>;
  ...
  it("treats renamed postgres migrations as already applied", async () => {
    await withTempMigrations(
      {
        "020_node_pairing_trust_allowlist.sql": "INVALID SQL",
      },
      async (dir) => {
        const calls: Array<{ text: string; params?: unknown[] }> = [];
        const client = {
          query: async (text: string, params?: unknown[]) => {
            ...
            if (trimmed.startsWith("SELECT name FROM _migrations")) {
              return { rows: [{ name: "019_node_pairing_trust_allowlist.sql" }] };
            }

            if (trimmed === "INVALID SQL") {
        ...
        expect(calls.some((c) => c.text === "INVALID SQL")).toBe(false);
        expect(
          calls.some(
            (c) =>
              c.text.startsWith("INSERT INTO _migrations") &&
              c.params?.[0] === "020_node_pairing_trust_allowlist.sql",
          ),
        ).toBe(true);
      },
    );
