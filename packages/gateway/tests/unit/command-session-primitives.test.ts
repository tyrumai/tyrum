...
describe("session command primitives", () => {
  ...
  it("supports /new with empty messages and empty prompt context", async () => {
    ...
    const payload = result.data as { session_id: string; thread_id: string };
  ...
  it("supports /compact via the runtime and leaves durable messages intact", async () => {
    ...
    const result = await executeCommand("/compact", {
      ...
      agents: {
        getRuntime: async () => ({
          compactSession: async ({ sessionId }: { sessionId: string }) => {
            const dal = new SessionDal(db!, new IdentityScopeDal(db!), new ChannelThreadDal(db!));
  ...
  it("supports /reset and clears session messages plus prompt context", async () => {
    db = openTestSqliteDb();
    const session = await support.ensureSession(db, {
      agentKey: "default",
      channel: "ui",
