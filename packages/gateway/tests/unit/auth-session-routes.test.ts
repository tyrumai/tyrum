/**
 * auth-session.ts — unit tests for auth session routes.
 *
 * Tests the branch paths for POST /auth/session and POST /auth/logout.
 */

import { describe, expect, it, vi } from "vitest";
import { Hono } from "hono";
import { createAuthSessionRoutes } from "../../src/routes/auth-session.js";
...
function buildApp(authTokens: AuthTokenService): Hono {
  const app = new Hono();
  app.route("/", createAuthSessionRoutes({ authTokens }));
  return app;
}
...
describe("POST /auth/session", () => {
  ...
  it("returns 403 for system tokens (tenant_id === null)", async () => {
    ...
    const body = (await res.json()) as { message: string };
    expect(body.message).toBe("system tokens cannot start sessions");
  });
  ...
  it("sets secure=true cookie for HTTPS requests", async () => {
    ...
    const app = buildApp(authTokens);
    const res = await app.request("https://example.com/auth/session", {
      method: "POST",
      body: JSON.stringify({ token: "admin-token" }),
      headers: { "content-type": "application/json" },
    });
