...
import * as commandSupport from "./command-conversation-primitives.test-support.js";
import {
...
import {
  ...
  textFromTurnRequest,
  textParts,
} from "./agent-behavior-conversation-boundaries.test-support.js";

describe("Agent behavior - conversation boundaries", () => {
  ...
  it("isolates DM conversation recall by thread when no durable memory is enabled", async () => {
    ...
    try {
      ...
      await db.run(
        `INSERT INTO conversation_leases (
         ...
         VALUES (?, ?, ?, ?)`,
        [DEFAULT_TENANT_ID, first.inbox.key, "worker-1", Date.now() + 60_000],
      );
      ...
      const signal = await db.get<{ kind: string; message_text: string }>(
        ...
        [DEFAULT_TENANT_ID, first.inbox.key],
      );
  ...
  it("does not reconstruct cleared conversation context from retained channel logs", async () => {
    ({ homeDir, container } = await setupTestEnv());

    const conversation = await commandSupport.ensureConversation(container.db, {
      agentKey: "default",
      channel: "telegram",
