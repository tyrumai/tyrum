...
} from "../../src/modules/agent/runtime/conversation-compaction-service.js";
...
describe("shouldCompactConversationForUsage", () => {
  beforeEach(() => {
  ...
  it("retries when the first checkpoint drops a critical identifier", async () => {
    ...
    const result = await compactConversationWithResolvedModel({
      container: { db: {}, logger: { warn: vi.fn() } } as never,
      conversationDal: { replaceContextState } as never,
      ctx: {
        config: {
  ...
  it("falls back to deterministic checkpointing when model compaction fails", async () => {
    ...
    expect(replaceContextState).toHaveBeenCalledOnce();
    expect(logger.warn).toHaveBeenCalledWith("agents.conversation_compaction_failed", {
      conversation_id: "conversation-1",
      error: "rate limited",
    });
  });
