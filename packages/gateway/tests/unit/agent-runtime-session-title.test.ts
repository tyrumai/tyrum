...
describe("AgentRuntime session title generation", () => {
  ...
  it("generates a title after the first turn and does not overwrite it later", async () => {
    homeDir = await mkdtemp(join(tmpdir(), "tyrum-session-title-"));
    ...
    await runtime.turn({
      channel: "test",
      thread_id: "thread-session-title",
      message: "Please debug the webhook retry failure in production logs.",
    });

    let sessions = await container.sessionDal.list({ connectorKey: "test", limit: 10 });
    expect(sessions.sessions[0]?.title).toBe("Investigate webhook retry failures");
    ...
    sessions = await container.sessionDal.list({ connectorKey: "test", limit: 10 });
    expect(sessions.sessions[0]?.title).toBe("Investigate webhook retry failures");
