...
describe("AgentRuntime - tool tracking, memory, and conversation queue signals", () => {
  ...
  it("logs when subagent execution profile resolution fails", async () => {
    ...
    const profile = await resolveExecutionProfile(
      ...
      { queueTarget: { key }, metadata: { subagent_id: subagentId } },
    );
  ...
  it("clears conversation interrupt signals when the conversation lease is released", async () => {
    homeDir = await mkdtemp(join(tmpdir(), "tyrum-agent-runtime-"));
    ...
    await container.db.run(
      `INSERT INTO conversation_leases (tenant_id, conversation_key, lease_owner, lease_expires_at_ms)
       VALUES (?, ?, ?, ?)`,
      [DEFAULT_TENANT_ID, key, leaseOwner, nowMs + 60_000],
    );

    const signals = new ConversationQueueSignalDal(container.db);
    ...
    await runtime1.turn({
      ...
      thread_id: "thread-1",
      message: "hello",
      metadata: { tyrum_key: key },
    });

    const remaining = await container.db.get<{ n: number }>(
      "SELECT COUNT(*) AS n FROM conversation_queue_signals WHERE conversation_key = ?",
      [key],
    );
    ...
    await container.db.transaction(async (tx) => {
      ...
      if (res.changes === 1) {
        await tx.run("DELETE FROM conversation_queue_signals WHERE conversation_key = ?", [key]);
      }
    });
