...
export function createConversationDal(deps: ProtocolDeps): ConversationDal {
  if (!deps.db) {
    throw new Error("missing db");
  }
  const identityScopeDal =
    deps.identityScopeDal ?? new IdentityScopeDal(deps.db, { cacheTtlMs: 60_000 });
  return new ConversationDal(deps.db, identityScopeDal, new ChannelThreadDal(deps.db));
}

export function conversationErrorResponse(params: {
  deps: ProtocolDeps;
  err: unknown;
