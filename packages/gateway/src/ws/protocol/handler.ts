...
export async function handleClientMessage(
  ...
  const subagentRes = await handleSubagentMessage(client, msg, deps);
  if (subagentRes) return subagentRes;
  ...
  const memoryRes = await handleMemoryMessage(client, msg, deps);
