...
import { wireContainer, type GatewayContainer } from "../container.js";
...
type SecretProviderForTenant = Awaited<
  ReturnType<typeof createDbSecretProviderFactory>
>["secretProviderForTenant"];
type LifecycleHooks = Awaited<ReturnType<typeof loadLifecycleHooksFromHome>>;

interface GatewayBootContext {
  instanceId: string;
  role: GatewayRole;
  tyrumHome: string;
  host: string;
...
async function createProtocolRuntime(
  context: GatewayBootContext,
  otel: OtelRuntime,
...
async function startEdgeRuntime(
  context: GatewayBootContext,
