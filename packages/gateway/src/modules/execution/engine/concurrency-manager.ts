...
export async function tryAcquireConcurrencySlotTx(
  ...
  await ensureConcurrencySlotsTx(tx, opts.scope, opts.scopeId, opts.limit);
...
export async function tryAcquireConcurrencyForAttemptTx(
  ...
  if (!(await claimScope("global", "global", globalLimit))) {
    await releaseConcurrencySlotsTx(tx, opts.attemptId, opts.nowIso, concurrencyLimits);
    return false;
  }
...
export async function releaseLaneAndWorkspaceLeasesTx(
  ...
  await releaseLaneLeaseTx(tx, { key: opts.key, lane: opts.lane, owner: opts.owner });
  await releaseWorkspaceLeaseTx(tx, { workspaceId: opts.workspaceId, owner: opts.owner });
}
...
export async function touchLaneLeaseTx(
  ...
  await tx.run(
    `UPDATE lane_leases
     ...
     WHERE key = ? AND lane = ? AND lease_owner = ?`,
    [opts.expiresAtMs, opts.key, opts.lane, opts.owner],
  );
}
