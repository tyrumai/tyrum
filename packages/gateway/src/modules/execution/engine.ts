...
function buildExecCommandFromCliArgs(args: unknown): string | undefined {
  ...
  const cmdArgs = Array.isArray(obj["args"]) ? obj["args"].filter((v): v is string => typeof v === "string") : [];
  return [cmd, ...cmdArgs].join(" ").trim();
}

function mapActionToPolicyTool(action: ActionPrimitiveT): {
  toolId: string;
  toolArgs: unknown;
  url?: string;
} {
  if (action.type === "Http") {
    ...
    const url =
      args && typeof args === "object" && !Array.isArray(args) && typeof (args as Record<string, unknown>)["url"] === "string"
        ? String((args as Record<string, unknown>)["url"])
        : undefined;
    return { toolId: "tool.http.fetch", toolArgs: args, url };
  }

  if (action.type === "CLI") {
    const command = buildExecCommandFromCliArgs(action.args) ?? "";
    return { toolId: "tool.exec", toolArgs: { command } };
  }

  return { toolId: `action.${action.type}`, toolArgs: action.args ?? {} };
}
...
function parseConcurrencyLimitsFromEnv(logger?: Logger): ExecutionConcurrencyLimits | undefined {
...
export class ExecutionEngine {
  ...
  constructor(opts: {
    ...
    this.concurrencyLimits =
      ...
      parseConcurrencyLimitsFromEnv(opts.logger);
  }
  ...
  private async tickWithLaneLease(
    ...
    const outcome = await this.db.transaction(async (tx) => {
      ...
      const budgets = budgetsRaw && typeof budgetsRaw === "object" ? (budgetsRaw as Record<string, unknown>) : undefined;
      ...
      if (budgets && !budgetOverridden) {
        ...
        for (const row of costRows) {
          ...
          const totalTokens =
            ...
            ((normalizeNonnegativeInt(c["input_tokens"]) ?? 0) + (normalizeNonnegativeInt(c["output_tokens"]) ?? 0));
          spentTotalTokens += totalTokens;
        }

        const reasons: string[] = [];
        if (maxUsdMicros !== undefined && spentUsdMicros > maxUsdMicros) {
          reasons.push(`spent_usd_micros=${String(spentUsdMicros)} > max_usd_micros=${String(maxUsdMicros)}`);
        }
        if (maxTotalTokens !== undefined && spentTotalTokens > maxTotalTokens) {
          reasons.push(`spent_total_tokens=${String(spentTotalTokens)} > max_total_tokens=${String(maxTotalTokens)}`);
        }
            ...
            if (updated.changes === 1) {
              ...
              await tx.run(
                ...
                [
                  ...
                  clock.nowIso,
                  clock.nowIso,
                  this.redactText(`policy denied ${mapped.toolId}`).trim() || "policy denied",
                  JSON.stringify(
                    this.redactUnknown({
            ...
            if (!alreadyApproved) {
              ...
              const paused = await this.pauseRunForApproval(
                ...
                },
                {
                  ...
                  detail: `policy requires approval for '${mapped.toolId}' (${toolMatchTarget || "unknown"})`,
                  context: {
  ...
  private async maybeRetryOrFailStep(opts: {
    ...
    if (opts.attemptNum < maxAttempts) {
      const step = await tx.get<{ idempotency_key: string | null; action_json: string; step_index: number }>(
      ...
      const planId =
        (job?.trigger_json ? parsePlanIdFromTriggerJson(job.trigger_json) : undefined) ?? opts.runId;
  ...
  private async tryAcquireConcurrencyForAttemptTx(
    ...
    const capabilityLimit =
      opts.capability && limits.perCapability
        ? limits.perCapability[opts.capability]
        : undefined;
