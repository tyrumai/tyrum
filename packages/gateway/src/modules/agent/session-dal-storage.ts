...
export function buildSessionSelectSql(kind: StateStoreKind, alias = "s"): string {
  return `${alias}.tenant_id,
       ${alias}.conversation_id AS session_id,
       ${alias}.conversation_key AS session_key,
       ${alias}.agent_id,
       ${alias}.workspace_id,
...
export function buildSessionWithDeliverySql(kind: StateStoreKind): string {
  return `SELECT ${buildSessionSelectSql(kind, "s")},
       ag.agent_key,
       ws.workspace_key,
       ca.connector_key,
       ca.account_key,
