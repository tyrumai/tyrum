...
export interface ResolveConversationModelDeps {
...
export async function resolveCandidates(
  ...
  const override = await new ConversationModelOverrideDal(deps.container.db).get({
  ...
  const assignedPreset =
    conversationPreset || !executionProfileId
      ? undefined
      : await (async () => {
  ...
  const candidateInputs: CandidateInput[] = (() => {
    ...
    const presetCandidate = (
      preset: NonNullable<typeof conversationPreset> | NonNullable<typeof assignedPreset>,
    ): CandidateInput => ({
      rawModelId: `${preset.provider_key}/${preset.model_id}`,
      optionsOverride: preset.options,
    });

    if (conversationPreset) return [presetCandidate(conversationPreset), ...baseCandidates];
