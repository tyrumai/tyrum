...
export async function syncWorkflowRunStepCostFromAttemptsTx(
  tx: SqlDb,
  input: {
    tenantId: string;
  ...
  const rows = await tx.all<CostJsonRow>(
    `SELECT cost_json
     FROM execution_attempts
     WHERE tenant_id = ?
       AND step_id = ?
       AND cost_json IS NOT NULL`,
    [input.tenantId, input.stepId],
  );
  if (rows.length === 0) {
    return;
  }
  ...
  const costs = rows
    .map((row) => parseAttemptCostJson(row.cost_json))
    .filter((cost): cost is AttemptCostT => cost !== undefined);
  if (costs.length === 0) {
    return;
  }
  ...
  await tx.run(
    `UPDATE workflow_run_steps
     ...
     WHERE tenant_id = ?
       ...
       AND step_index = ?`,
    [
      JSON.stringify(sumAttemptCosts(costs)),
      input.updatedAtIso,
      input.tenantId,
      input.workflowRunId,
      input.stepIndex,
