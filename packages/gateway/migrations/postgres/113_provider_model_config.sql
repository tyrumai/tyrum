...
ALTER TABLE auth_profiles ADD COLUMN config_json TEXT NOT NULL DEFAULT '{}';
...
CREATE TABLE configured_model_presets (
  ...
  CONSTRAINT configured_model_presets_tenant_fk
...
CREATE TABLE execution_profile_model_assignments (
  ...
  CONSTRAINT execution_profile_model_assignments_tenant_fk
    FOREIGN KEY (tenant_id) REFERENCES tenants(tenant_id) ON DELETE CASCADE,
  CONSTRAINT execution_profile_model_assignments_preset_fk
...
INSERT INTO execution_profile_model_assignments (tenant_id, execution_profile_id, preset_key)
SELECT tenant_id, 'interaction', 'legacy-openai-gpt-4-1-medium' FROM tenants
ON CONFLICT (tenant_id, execution_profile_id) DO NOTHING;
