...
static AUTHORIZATION_COUNTER: Lazy<Counter<u64>> = Lazy::new(|| {
    global::meter("tyrum-wallet")
        .u64_counter("tyrum_wallet_authorizations_total")
        .with_description("Number of authorization decisions emitted by the Tyrum wallet service")
        .build()
});
...
impl Drop for TelemetryGuard {
    fn drop(&mut self) {
        ...
        if let Err(error) = self.meter_provider.shutdown() {
            eprintln!("meter provider shutdown error: {error}");
        }
        if let Err(error) = self.logger_provider.shutdown() {
            eprintln!("logger provider shutdown error: {error}");
        }
    }
}

pub fn record_authorization(decision: AuthorizationDecision) {
    let outcome = match decision {
        AuthorizationDecision::Approve => "approve",
        AuthorizationDecision::Escalate => "escalate",
        AuthorizationDecision::Deny => "deny",
    };

    AUTHORIZATION_COUNTER.add(1, &[KeyValue::new("wallet.decision", outcome)]);
}
