...
{
    let span = info_span!(
        "executor.web.attempt",
        attempt,
        max_attempts,
        target_host = context.host(),
        target_scheme = context.scheme(),
        outcome = field::Empty,
        error = field::Empty,
...
fn record_metrics(context: &AttemptContext, attempt: u32, outcome: &str, duration: Duration) {
    ...
    let attributes = [
        ...
        KeyValue::new("executor.web.outcome", outcome.to_string()),
        KeyValue::new("executor.web.attempt_number", attempt as i64),
    ];
...
fn metrics_instruments() -> MetricsInstruments {
    ...
    let instruments = MetricsInstruments {
        duration: meter
            ...
            .with_unit("s")
            .with_description("Duration of Playwright executor attempts")
            .build(),
        count: meter
            .u64_counter(COUNT_METRIC_NAME)
            .with_description("Attempts performed by the Playwright executor")
            .build(),
    };
