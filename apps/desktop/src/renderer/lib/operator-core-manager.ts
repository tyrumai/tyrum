...
export type DesktopOperatorCoreFactory = (options: {
  wsUrl: string;
  httpBaseUrl: string;
...
export type DesktopOperatorCoreManagerOptions = {
  wsUrl: string;
  httpBaseUrl: string;
  baselineAuth: OperatorAuthStrategy;
  adminModeStore: AdminModeStore;
  createCore?: DesktopOperatorCoreFactory;
};

export type DesktopOperatorCoreManager = {
...
export function createDesktopOperatorCoreManager(
  options: DesktopOperatorCoreManagerOptions,
): DesktopOperatorCoreManager {
  const createCore: DesktopOperatorCoreFactory =
    options.createCore ??
    ((coreOptions) =>
      createOperatorCore({
