...
describe("NodeRuntime remote TLS pinning", () => {
  it("passes remote.tlsCertFingerprint256/tlsAllowSelfSigned through to TyrumClient", async () => {
    vi.resetModules();
  ...
  it("connects the desktop runtime as a node even when legacy config has device.enabled=false", async () => {
    vi.resetModules();
