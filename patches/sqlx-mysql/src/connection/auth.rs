...
impl AuthPlugin {
    pub(super) async fn scramble(
        ...
        match self {
            ...
            AuthPlugin::Sha256Password => encrypt_rsa(stream, 0x01, password).await,
        ...
        match self {
            AuthPlugin::CachingSha2Password if packet[0] == 0x01 => {
                match packet[1] {
                    ...
                    // AUTH_CONTINUE
                    0x04 => {
                        let payload = encrypt_rsa(stream, 0x02, password).await?;
...
async fn encrypt_rsa(
    ...
    // RSA-based SHA256 auth without TLS is intentionally disabled in this fork to
    // avoid depending on the vulnerable `rsa` crate. Encourage users to rely on TLS.
    Err(err_protocol!(
        "sha256_password authentication requires TLS in this build; \
         retry with TLS enabled (public key flow disabled for security)"
    ))
}
