...
flowchart TB
  ...
  ASM --> INF["Model inference"]
  INF --> ACT["Tool execution<br/>(0..N calls)"]
  ACT --> OUT["Stream replies + evidence"]
  OUT --> PERSIST["Persist state<br/>(transcripts, events, memory)"]
```

## Serialization guarantee (target)

- Runs are serialized per session key to prevent tool and transcript races.
