...
Tyrum runs as a single-host local installation or as a horizontally scalable deployment with replicated edges, workers, and lease-coordinated schedulers backed by a durable StateStore.

## Mission

This page defines how Tyrum preserves the same logical architecture across deployment sizes. It explains which coordination primitives must exist in every deployment and how durability, routing, and execution stay correct as replica counts change.

## Responsibilities

- Define the logical deployment building blocks that appear in all runtime shapes.
- Explain how state, leases, routing, and event delivery behave in single-host and clustered installs.
- Set the invariants for workers, schedulers, workspace durability, and failover.
- Provide the parent overview for lower-level storage, backplane, and operational maintenance docs.

## Non-responsibilities

- This page does not define the typed protocol itself; see [Protocol](/architecture/protocol).
- This page does not replace component-level execution or approval docs; it explains how those components behave under scale and failure.

## Boundary and ownership

- **Inside the boundary:** deployment roles, StateStore expectations, event backplane behavior, worker/scheduler coordination, and failover semantics.
- **Outside the boundary:** product-level UX semantics and component-specific business logic.
...
- **Inputs:** deployment shape, storage backend choice, replica counts, failure conditions, and runtime traffic.
- **Outputs:** durable coordination behavior, failover semantics, routing expectations, and operator-visible recovery behavior.
- **Dependencies:** gateway, execution engine, protocol, StateStore, backplane, workspace storage, and operational tooling.

## Invariants and constraints

- Durable state must survive restarts and remain the source of truth.
- At-least-once delivery, retries, and leases must remain correct across both small and large deployments.
- Workspace durability and single-writer semantics must hold without weakening operator expectations.

## Failure and recovery

- **Failure modes:** edge crashes, worker crashes, scheduler failover, transient database failures, and network partitions.
- **Recovery model:** durable state, leases, idempotency, and backplane/event replay make failover correct rather than merely available.

## Security and policy boundaries

- Secrets-by-handle, policy checks, and approval semantics must remain identical across deployment sizes.
- Scaling must not introduce alternate execution paths that bypass audit, policy, or durable coordination.

## Key decisions and tradeoffs

- **One logical architecture across deployment sizes:** single-host installs use the same core coordination model as clustered deployments.
- **Durable primitives over in-memory shortcuts:** leases, outbox delivery, and persistent routing keep scale behavior predictable.
- **Workspace access as an explicit boundary:** ToolRunner owns workspace mounts so clustered execution does not rely on unsafe shared POSIX assumptions.
...
- **Lane serialization:** execution is serialized per `(session_key, lane)` (see [Sessions and Lanes](/architecture/sessions-lanes)).
- **Secrets by handle:** raw secret values do not enter model context; executors resolve handles at the last responsible moment (see [Secrets](/architecture/secrets)).
- **Observable execution:** long-running work emits events and can be inspected/replayed from durable logs (see [Execution engine](/architecture/execution-engine)).
...
To keep behavior consistent when scaling up, deployments route updates through an **event backplane** abstraction: in a single-host deployment this may be in-process, while in multi-instance deployments it becomes a shared backplane.
...
The exact mechanism can vary (row-level locks, advisory locks, lease rows), but the observable behavior must match the [Sessions and Lanes](/architecture/sessions-lanes) guarantee.
...
In clustered deployments, **durable workspaces** interact with Kubernetes volume semantics. To keep the single-host and cluster behaviors aligned while avoiding RWX requirements, Tyrum uses a simple rule:
...
- network partitions between components (edge-to-database and worker-to-database)
