...
- Treat tool outputs (especially from web pages and channels) as untrusted input and rely on provenance tagging plus policy rules to prevent prompt injection and unsafe tool chaining (see [Sandbox and policy](/architecture/sandbox-policy)).
...
- **`fs` (workspace filesystem):**
  ...
  - Example match targets: `read:src/main.ts`, `write:docs/architecture/scaling-ha/backplane.md`, `delete:tmp/output.log`.
- **`bash` / CLI execution:**
...
- **`fs`**: narrow prefixes like `write:src/generated/*` or `read:docs/architecture/gateway/*`.
