...
- The workspace filesystem is an operator-visible surface for files and artifacts; **long-term memory is stored in the StateStore** (see [Memory](/architecture/memory)).
...
- The system enforces this with StateStore-backed leases/claims and a ToolRunner boundary (see [Scaling and high availability](/architecture/scaling-ha)).
