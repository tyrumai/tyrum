...
Details: [Provider Auth and Onboarding](/architecture/auth) and [Secrets](/architecture/secrets).
...
Operator surfaces (`/status`, `/usage`, Configure > Providers, Configure > Models) expose the active model/provider, account state, and provider usage where available (see [Observability](/architecture/observability)).
