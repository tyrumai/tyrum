...
For schema-backed interfaces, the exported contracts in `packages/schemas` are the authoritative source of truth. Architecture docs may summarize or explain those interfaces, but the schema definitions govern the actual wire and validation behavior.
