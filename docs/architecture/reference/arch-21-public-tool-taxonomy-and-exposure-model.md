...
- The `mcp.` prefix stays public only for raw MCP-backed tools that are exposed as MCP inventory. Built-in MCP facades such as `websearch`, `webfetch`, and `codesearch` keep facade IDs, and built-in memory helpers move to `memory.*` as their canonical public IDs.
...
- Define `memory.seed`, `memory.search`, and `memory.write` as the canonical public IDs for built-in memory helpers. The currently shipped `mcp.memory.*` IDs become deprecated public aliases during the migration window tracked by later epic items.
...
- The action or verb belongs in the final segment: `artifact.describe`, `memory.write`, `tool.location.place.create`, `tool.automation.schedule.pause`, `workboard.item.list`, `mcp.github.search`.
...
| `memory` | `memory.<verb>` | `mcp.memory.seed`, `mcp.memory.search`, `mcp.memory.write` | **renamed** | `default` | Canonical public IDs are `memory.seed`, `memory.search`, `memory.write`; `mcp.memory.*` remains only as deprecated aliases during migration. |
...
- Follow-on migration issues must normalize public memory references from `mcp.memory.*` to `memory.*` across defaults, prompts, execution profiles, policy, bookkeeping, and docs.
